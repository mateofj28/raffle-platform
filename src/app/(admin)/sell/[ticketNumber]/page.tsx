...
export default function SellTicketPage() {
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      if (!snap.empty) {
        const r = snap.docs[0];
        setActiveRaffle({ id: r.id, name: r.data().name, status: r.data().status, ticketPrice: r.data().ticketPrice, totalTickets: r.data().totalTickets, semester: r.data().semester, endDate: r.data().endDate });
      } else {
